...
async function getAllPosts(req, res) {
  try {
    ...
    let query = `
      ...
      LEFT JOIN admins a 
        ON CONVERT(p.author_id USING utf8mb4) COLLATE utf8mb4_unicode_ci = CONVERT(a.id USING utf8mb4) COLLATE utf8mb4_unicode_ci
        AND p.author_type = 'admin' 
        AND a.status = 'active'
      LEFT JOIN users u 
        ON CONVERT(p.author_id USING utf8mb4) COLLATE utf8mb4_unicode_ci = CONVERT(u.id USING utf8mb4) COLLATE utf8mb4_unicode_ci
...
async function getRelatedPosts(req, res) {
  try {
    ...
    const categoryIdNum = String(category_id || '').trim();
    const excludeIdNum = resolvePostId(exclude_id) || String(exclude_id || '').trim();
...
async function addComment(req, res) {
  try {
    ...
    const postIdNum = String(post_id || '').trim();
    const parentIdNum = parent_id ? String(parent_id || '').trim() : null;
    ...
    await db.execute(
      `INSERT INTO comments 
        (id, post_id, user_id, author_name, author_avatar, content, parent_id)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
      [commentId, postIdNum, userId, authorName, authorAvatar, content, parentIdNum]
    );
...
async function createPost(req, res) {
  ...
  try {
    ...
    // Image path from multer/storageFactory
    const featured_image = req.file
      ? (req.file.storedUrl || (req.file.filename ? `uploads/${req.file.filename}` : null))
    ...
    await db.execute(
      `INSERT INTO posts 
      (id, title, slug, excerpt, content, category_id, author_id, author_type, featured_image, status, published_at)
       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
      [
        postId,
        title,
    ...
    if (parsedTags.length > 0) {
      for (let tagName of parsedTags) {
        ...
        if (existingTag.length > 0) {
          ...
          tagId = createUuid();
          await db.execute('INSERT INTO tags (id, name, slug) VALUES (?, ?, ?)', [tagId, tagName, tagSlug]);
        }
...
async function updatePost(req, res) {
  try {
    ...
    // Check authorization
    const isOwner = String(post.author_id) === userId && post.author_type === userType;
    ...
    const categoryIdNum = category_id !== undefined ? (category_id ? String(category_id) : null) : post.category_id;
