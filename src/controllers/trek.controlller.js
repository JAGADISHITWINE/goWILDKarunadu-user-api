...
async function getDashboardData(req, res) {
  ...
  try {
    ...
    const limit = (!isNaN(limitParam) && limitParam > 0) ? Math.min(limitParam, 50) : 8;
      ...
      SELECT
        ...
        (
          ...
          FROM (
            ...
            WHERE CONVERT(h.trek_id USING utf8mb4) COLLATE utf8mb4_unicode_ci = CONVERT(t.id USING utf8mb4) COLLATE utf8mb4_unicode_ci
            ORDER BY h.id ASC
            LIMIT 3
          ) hh
        ) AS highlights
      FROM trek_batches b
      INNER JOIN treks t ON CONVERT(t.id USING utf8mb4) COLLATE utf8mb4_unicode_ci = CONVERT(b.trek_id USING utf8mb4) COLLATE utf8mb4_unicode_ci
      LEFT JOIN (
        SELECT
          tb.trek_id,
        ...
        GROUP BY tb.trek_id
      ) rt ON CONVERT(rt.trek_id USING utf8mb4) COLLATE utf8mb4_unicode_ci = CONVERT(t.id USING utf8mb4) COLLATE utf8mb4_unicode_ci
      LEFT JOIN trek_engagement te ON CONVERT(te.trek_id USING utf8mb4) COLLATE utf8mb4_unicode_ci = CONVERT(t.id USING utf8mb4) COLLATE utf8mb4_unicode_ci
...
async function getTrekById(req, res) {
  ...
  try {
    ...
    // Get basic trek info
    const [[trek]] = await conn.query(
      "SELECT * FROM treks WHERE CONVERT(id USING utf8mb4) COLLATE utf8mb4_unicode_ci = CONVERT(? USING utf8mb4) COLLATE utf8mb4_unicode_ci",
      [trekId]
    );
    ...
    // 📈 Get elevation waypoints
    const [waypoints] = await conn.query(
      "SELECT km, elevation, name, icon, note FROM trek_elevation_waypoints WHERE trek_id = ? ORDER BY display_order ASC, km ASC",
      [trekId]
    ).catch(() => [[]]);
    trek.elevationWaypoints = waypoints.map(w => ({
      km: Number(w.km || 0),
      elevation: Number(w.elevation || 0),
      name: w.name,
      icon: w.icon || '⛰️',
      note: w.note || ''
    }));
  ...
  try {
    ...
    const [treks] = await conn.query(`
      SELECT 
        ...
        MAX(COALESCE(rt.avg_rating, 0)) AS rating,
        MAX(COALESCE(rt.review_count, 0)) AS reviews,
        MAX(COALESCE(te.total_views, 0)) AS views,
        MAX(t.created_at) AS created_at,
        MAX(t.updated_at) AS updated_at
      FROM treks t
      LEFT JOIN trek_batches b ON CONVERT(b.trek_id USING utf8mb4) COLLATE utf8mb4_unicode_ci = CONVERT(t.id USING utf8mb4) COLLATE utf8mb4_unicode_ci
      LEFT JOIN (
        SELECT
          tb.trek_id,
          ROUND(AVG(rating), 1) AS avg_rating,
        ...
        FROM trek_ratings tr
        INNER JOIN bookings b ON b.id = tr.booking_id
        INNER JOIN trek_batches tb ON CONVERT(tb.id USING utf8mb4) COLLATE utf8mb4_unicode_ci = CONVERT(b.batch_id USING utf8mb4) COLLATE utf8mb4_unicode_ci
        GROUP BY tb.trek_id
      ) rt ON CONVERT(rt.trek_id USING utf8mb4) COLLATE utf8mb4_unicode_ci = CONVERT(t.id USING utf8mb4) COLLATE utf8mb4_unicode_ci
      LEFT JOIN trek_engagement te ON CONVERT(te.trek_id USING utf8mb4) COLLATE utf8mb4_unicode_ci = CONVERT(t.id USING utf8mb4) COLLATE utf8mb4_unicode_ci
      GROUP BY t.id
    ...
    for (const trek of treks) {
      ...
      trek.earliest_start_date = trek.earliest_start_date
        ? formatDateOnlyForMySQL(trek.earliest_start_date)
        : null;

      trek.latest_end_date = trek.latest_end_date
        ? formatDateOnlyForMySQL(trek.latest_end_date)
        : null;
    ...
    const [[activeCount]] = await conn.query(`
      ...
      FROM treks t
      JOIN trek_batches b ON CONVERT(b.trek_id USING utf8mb4) COLLATE utf8mb4_unicode_ci = CONVERT(t.id USING utf8mb4) COLLATE utf8mb4_unicode_ci
      WHERE b.status = 'active'
        AND b.start_date >= CURDATE()
        AND (b.available_slots - b.booked_slots) > 0
    `);
